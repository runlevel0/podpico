...
impl FileManager {
    ...
    pub async fn download_episode(
        ...
        log::info!("Starting download for episode {} from {} (User Story #3)", episode_id, episode_url);
        ...
        if file_path.exists() {
            log::info!("Episode {} already downloaded at {:?}", episode_id, file_path);
            
            // User Story #3 Acceptance Criteria: Progress indicator appears immediately
            let mut downloads = self.downloads.lock().await;
            downloads.insert(episode_id, DownloadProgress {
                episode_id,
                total_bytes: 0,
                downloaded_bytes: 0,
                percentage: 100.0,
        ...
        let result = self.download_with_progress(episode_url, &file_path, episode_id).await;

        match result {
            Ok(path) => {
                log::info!("Successfully downloaded episode {} to {}", episode_id, path);
                self.update_download_status_with_speed(episode_id, DownloadStatus::Completed, 100.0, 0, 0, 0.0).await;
                Ok(path)
            }
            Err(e) => {
                log::error!("Failed to download episode {}: {}", episode_id, e);
                self.update_download_status_with_speed(episode_id, DownloadStatus::Failed(e.to_string()), 0.0, 0, 0, 0.0).await;
                Err(e)
            }
        }
    }

    async fn download_with_progress(&self, url: &str, file_path: &PathBuf, episode_id: i64) -> Result<String, PodPicoError> {
        use tokio::io::AsyncWriteExt;
        use futures_util::StreamExt;

        // Update status to in progress
        self.update_download_status(episode_id, DownloadStatus::InProgress, 0.0, 0, 0).await;

        let response = self.client.get(url).send().await
            .map_err(|e| PodPicoError::NetworkError(format!("Failed to start download: {}", e)))?;
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| PodPicoError::NetworkError(format!("Download stream error: {}", e)))?;
            ...
            self.update_download_status_with_speed(episode_id, DownloadStatus::InProgress, percentage, downloaded, total_size, speed).await;
        }
    ...
    async fn update_download_status(&self, episode_id: i64, status: DownloadStatus, percentage: f64, downloaded: u64, total: u64) {
        let mut downloads = self.downloads.lock().await;
        if let Some(progress) = downloads.get_mut(&episode_id) {
            ...
            if percentage > 0.0 && percentage < 100.0 {
                ...
                if progress.speed_bytes_per_sec > 0.0 {
                    progress.eta_seconds = Some((remaining_bytes as f64 / progress.speed_bytes_per_sec) as u64);
                }
            }
        }
    }

    pub async fn update_download_status_with_speed(&self, episode_id: i64, status: DownloadStatus, percentage: f64, downloaded: u64, total: u64, speed: f64) {
        let mut downloads = self.downloads.lock().await;
        if let Some(progress) = downloads.get_mut(&episode_id) {
            progress.status = status;
            progress.percentage = percentage;
    ...
    async fn check_disk_space(&self, directory: &Path) -> Result<(), PodPicoError> {
        ...
        match tokio::fs::write(&temp_file, b"test").await {
            ...
            Err(e) => Err(PodPicoError::IoError(format!("Insufficient disk space or permissions: {}", e)))
        }
    }
...
mod tests {
    ...
    async fn test_user_story_3_episode_download_success() {
        ...
        let result = file_manager.download_episode(&url, episode_id, podcast_id).await;
        
        assert!(result.is_ok(), "Download should succeed");
        let file_path = result.unwrap();
        assert!(std::path::Path::new(&file_path).exists(), "Downloaded file should exist");
        ...
        if let Some(progress) = progress {
            assert_eq!(progress.status, DownloadStatus::Completed, "Download should be completed");
            assert_eq!(progress.percentage, 100.0, "Download should be 100% complete");
        }
    ...
    async fn test_user_story_3_download_progress_tracking() {
        ...
        file_manager.update_download_status_with_speed(episode_id, DownloadStatus::InProgress, 50.0, 512, 1024, 100.0).await;
        ...
        if let Some(progress) = progress {
            ...
            assert_eq!(progress.downloaded_bytes, 512, "Should have correct downloaded bytes");
            assert_eq!(progress.total_bytes, 1024, "Should have correct total bytes");
            assert_eq!(progress.status, DownloadStatus::InProgress, "Should be in progress");
        }
    }
    ...
    async fn test_user_story_3_download_already_exists() {
        ...
        tokio::fs::write(&existing_file, b"existing content").await.unwrap();

        // Verify file exists before test
        assert!(existing_file.exists(), "Test setup: existing file should exist at {:?}", existing_file);
        ...
        let result = file_manager.download_episode(test_url, episode_id, podcast_id).await;
        assert!(result.is_ok(), "Should return existing file path: {:?}", result);
        ...
        assert_eq!(content, b"existing content", "File content should be unchanged");
        
        let progress = file_manager.get_download_progress(episode_id).await;
        if let Some(progress) = progress {
            assert_eq!(progress.status, DownloadStatus::Completed, "Should be marked as completed");
    ...
    async fn test_file_manager_initialization() {
        ...
        assert!(result.is_ok(), "File manager should initialize successfully");
        assert!(file_manager.download_directory.exists(), "Download directory should be created");
    }
    ...
    async fn test_get_episode_path() {
        ...
        assert!(path.to_string_lossy().contains("1"), "Path should contain podcast ID");
        assert!(path.to_string_lossy().contains("123.mp3"), "Path should contain episode ID and extension");
    }
    ...
    async fn test_extract_filename_from_url() {
        ...
        let filename = file_manager.extract_filename_from_url("https://example.com/podcast/episode123.mp3", 456);
        ...
        let filename = file_manager.extract_filename_from_url("https://cdn.example.com/episodes/show_name_ep_042.mp3?token=abc", 42);
    ...
    async fn test_disk_space_check() {
        ...
        let result = file_manager.check_disk_space(&file_manager.download_directory).await;
        assert!(result.is_ok(), "Disk space check should succeed for valid directory");
    }
}
