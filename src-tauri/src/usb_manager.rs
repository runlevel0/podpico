...
impl UsbManager {
    ...
    pub async fn transfer_file(
        ...
        if !source_path_buf.exists() {
            self.update_transfer_status(&transfer_key, TransferStatus::Failed(
        ...
        if !device_path_buf.exists() {
            self.update_transfer_status(&transfer_key, TransferStatus::Failed(
                format!("USB device not found: {}", device_path)
            )).await;
            return Err(PodPicoError::UsbDeviceNotFound(device_path.to_string()));
        }

        // Get file size for progress tracking
        let metadata = fs::metadata(&source_path_buf).await
            .map_err(|e| PodPicoError::FileTransferFailed(format!("Cannot read source file: {}", e)))?;
        ...
        if let Ok(device_info) = self.get_device_info(device_path) {
            if device_info.available_space < file_size {
                ...
                self.update_transfer_status(&transfer_key, TransferStatus::Failed(error_msg.clone())).await;
        ...
        fs::create_dir_all(&usb_podcast_dir).await
            .map_err(|e| PodPicoError::FileTransferFailed(format!("Cannot create directory on USB: {}", e)))?;

        let destination_path = usb_podcast_dir.join(filename);

        // Update progress to InProgress
        self.update_transfer_progress(&transfer_key, file_size, 0, TransferStatus::InProgress).await;

        // User Story #9 Acceptance Criteria: Transfer with progress tracking and speed calculation
        let result = self.transfer_with_progress(&source_path_buf, &destination_path, &transfer_key, file_size).await;

        match result {
            Ok(_) => {
                self.update_transfer_status(&transfer_key, TransferStatus::Completed).await;
                log::info!("Successfully transferred {} to USB device", filename);
                Ok(())
            }
            Err(e) => {
                self.update_transfer_status(&transfer_key, TransferStatus::Failed(e.to_string())).await;
    ...
    async fn transfer_with_progress(
        ...
        let mut source_file = fs::File::open(source_path).await
            .map_err(|e| PodPicoError::FileTransferFailed(format!("Cannot open source file: {}", e)))?;

        let mut dest_file = fs::File::create(destination_path).await
            .map_err(|e| PodPicoError::FileTransferFailed(format!("Cannot create destination file: {}", e)))?;
...
mod tests {
    ...
    async fn test_user_story_8_device_structure_validation() {
        ...
        for device in &devices {
            ...
            assert!(!device.id.contains('\\'), "Device ID should not contain backslashes");
            ...
            // Connection status
            assert!(device.is_connected, "Detected device should be marked as connected");
        }
    }
    ...
    async fn test_user_story_9_transfer_progress_indicator() {
        ...
        let start_time = std::time::Instant::now();
        let result = usb_manager.transfer_file(test_source, test_device, test_filename).await;
        let elapsed = start_time.elapsed();
        
        // Should respond reasonably quickly (progress indicator appears immediately)
        assert!(elapsed.as_millis() < 1000, "Transfer should complete quickly for small test file");
        
        // Should succeed now that we've implemented it
        assert!(result.is_ok(), "Transfer should succeed for valid inputs: {:?}", result);
        
        // Verify file was transferred
        let dest_path = format!("{}/PodPico/{}", test_device, test_filename);
        assert!(std::path::Path::new(&dest_path).exists(), "File should exist on device after transfer");
    ...
    async fn test_user_story_9_transfer_speed_and_time_remaining() {
        ...
        assert!(result.is_ok(), "Transfer should succeed for valid large file: {:?}", result);
        
        // Verify file was transferred and has correct size
        let dest_path = format!("{}/PodPico/{}", test_device, test_filename);
        assert!(std::path::Path::new(&dest_path).exists(), "Large file should exist on device");
        
        let dest_metadata = std::fs::metadata(&dest_path).unwrap();
        assert_eq!(dest_metadata.len(), 100 * 1024, "Transferred file should have correct size");
    ...
    async fn test_user_story_9_successful_transfer_indicator() {
        ...
        assert!(result.is_ok(), "Transfer should succeed and return success status: {:?}", result);
        
        // Verify file exists on device (indicates successful transfer)
        let dest_path = format!("{}/PodPico/{}", test_device, test_filename);
        assert!(std::path::Path::new(&dest_path).exists(), "File should exist on device after successful transfer");
        
        // Verify content is correct
        let transferred_content = std::fs::read_to_string(&dest_path).unwrap();
        assert_eq!(transferred_content, "successful episode content", "Transferred content should match original");
    ...
    async fn test_user_story_9_transfer_error_handling() {
        ...
        let result = usb_manager.transfer_file(nonexistent_source, test_device, test_filename).await;
        ...
        assert!(error_message.contains("Source file not found"), "Error should mention source file not found");
        assert!(error_message.contains(nonexistent_source), "Error should include the problematic path");
    ...
    async fn test_user_story_9_insufficient_space_warning() {
        ...
        assert!(result.is_ok(), "Transfer should succeed for small file: {:?}", result);
        ...
        let dest_path = format!("{}/PodPico/{}", test_device, test_filename);
        assert!(std::path::Path::new(&dest_path).exists(), "File should be transferred successfully");
    ...
    async fn test_user_story_9_transfer_file_organization() {
        ...
        assert!(result.is_ok(), "Transfer should succeed with proper organization: {:?}", result);
        
        // Verify files are organized in PodPico directory structure
        let podpico_dir = format!("{}/PodPico", test_device);
        assert!(std::path::Path::new(&podpico_dir).exists(), "PodPico directory should be created");
        assert!(std::path::Path::new(&podpico_dir).is_dir(), "PodPico should be a directory");
        
        let dest_path = format!("{}/{}", podpico_dir, test_filename);
        assert!(std::path::Path::new(&dest_path).exists(), "File should be in PodPico directory");
        
        // Verify content is preserved
        let content = std::fs::read_to_string(&dest_path).unwrap();
        assert_eq!(content, "organized episode content", "Content should be preserved during organized transfer");
    ...
    async fn test_user_story_9_transfer_performance_requirements() {
        ...
        assert!(elapsed.as_secs() < 10, "Transfer should complete within 10 seconds for 50KB file");
        
        // Should succeed
        assert!(result.is_ok(), "Performance test transfer should succeed: {:?}", result);
        
        // Verify file integrity
        let dest_path = format!("{}/PodPico/{}", test_device, test_filename);
        let dest_metadata = std::fs::metadata(&dest_path).unwrap();
        assert_eq!(dest_metadata.len(), 50 * 1024, "Transferred file should maintain correct size");
        
        // Calculate approximate transfer rate
        let transfer_rate = (50 * 1024) as f64 / elapsed.as_secs_f64(); // bytes per second
        assert!(transfer_rate > 1000.0, "Transfer rate should be at least 1KB/s, got {:.2} bytes/s", transfer_rate);
