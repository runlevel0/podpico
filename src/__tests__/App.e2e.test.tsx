    ...
    it('should download episode from start to finish with real backend', async () => {
      ...
      await waitFor(
        () => {
          ...
          const episodeItems = episodeElements.filter(
            el => el.className && el.className.includes('episode-item')
          )
      ...
      const episodeItems = screen
        .getAllByRole('generic')
        .filter(el => el.className && el.className.includes('episode-item'))
      ...
      await waitFor(
        ...
        { timeout: 15000 }
      ) // Allow up to 15 seconds for download to start showing progress
      ...
      try {
        ...
        if (episodeItems.length > 0) {
          ...
          await waitFor(
            () => {
              expect(
                screen.getByText(/Download failed|Error|Failed/)
              ).toBeInTheDocument()
      ...
      if (episodeItems.length > 0) {
        ...
        if (episodeItems.length > 1) {
          ...
          await waitFor(
            () => {
              expect(
                screen.getByText('📥 Download Episode')
              ).toBeInTheDocument()
            },
            { timeout: 2000 }
          )
      ...
      if (episodeItems.length > 0) {
        ...
        await waitFor(
          () => {
            ...
            const speedElements = screen.getAllByText(
              /\d+(\.\d+)?\s*(KB|MB)\/s/
            )
  ...
  describe('File System Integration', () => {
    it('should create actual files and show correct file paths', async () => {
      ...
      if (episodeItems.length > 0) {
        ...
        const downloadedEpisode = (episodes as any[]).find(
          ep => ep.id === testEpisodeId
        )
