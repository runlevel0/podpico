...
describe('SafeHtmlRenderer Component', () => {
  describe('HTML Rendering', () => {
    ...
    it('renders HTML links correctly', () => {
      const htmlContent = '<p>Visit <a href="https://example.com">our website</a> for more info.</p>'
      render(<SafeHtmlRenderer htmlContent={htmlContent} />)
    ...
    it('renders emphasis and strong tags correctly', () => {
      const htmlContent = '<p>This is <em>emphasized</em> and this is <strong>strong</strong> text.</p>'
      render(<SafeHtmlRenderer htmlContent={htmlContent} />)
    ...
    it('renders line breaks correctly', () => {
      ...
      const { container } = render(<SafeHtmlRenderer htmlContent={htmlContent} />)
  ...
  describe('Security and Sanitization', () => {
    it('removes dangerous script tags', () => {
      ...
      const { container } = render(<SafeHtmlRenderer htmlContent={maliciousContent} />)
    ...
    it('removes dangerous iframe tags', () => {
      const maliciousContent = '<p>Safe content</p><iframe src="javascript:alert(1)"></iframe>'
      const { container } = render(<SafeHtmlRenderer htmlContent={maliciousContent} />)
    ...
    it('preserves safe attributes on links', () => {
      const htmlContent = '<a href="https://example.com" target="_blank" rel="noopener">Safe link</a>'
      render(<SafeHtmlRenderer htmlContent={htmlContent} />)
  ...
  describe('Search Highlighting', () => {
    ...
    it('highlights search terms in HTML content', () => {
      const htmlContent = '<p>This episode covers <em>React</em> testing fundamentals.</p>'
    ...
    it('highlights multiple occurrences of search terms', () => {
      const content = 'React is great. Learn React development with React tutorials.'
      render(<SafeHtmlRenderer htmlContent={content} searchQuery="React" />)
    ...
    it('does not highlight when search query is empty', () => {
      ...
      const { container } = render(<SafeHtmlRenderer htmlContent={content} searchQuery="" />)
    ...
    it('escapes special regex characters in search query', () => {
      ...
      render(<SafeHtmlRenderer htmlContent={content} searchQuery="$variables" />)
  ...
  describe('Real-world Podcast Description Examples', () => {
    it('handles complex podcast description with paragraphs and emphasis', () => {
      ...
      expect(screen.getByText(/Church of the City New York/)).toBeInTheDocument()
    })

    it('handles podcast description with links and line breaks', () => {
      const podcastDescription = 'Find <a href="https://open.spotify.com/show/2naS9Y5nWSli9vi9bOvGxp">all the episodes from this season here</a>. And <a href="https://www.npr.org/series/1015448333/planet-money-summer-school">past seasons here</a>. <br/><br/>Planet Money Summer School has arrived at the birth of the United States and the chance to set up a whole new economy from scratch.'
      
      render(<SafeHtmlRenderer htmlContent={podcastDescription} />)
      
      const spotifyLink = screen.getByRole('link', { name: 'all the episodes from this season here' })
      expect(spotifyLink).toHaveAttribute('href', 'https://open.spotify.com/show/2naS9Y5nWSli9vi9bOvGxp')
      
      const nprLink = screen.getByRole('link', { name: 'past seasons here' })
      expect(nprLink).toHaveAttribute('href', 'https://www.npr.org/series/1015448333/planet-money-summer-school')
    ...
    it('handles search highlighting in complex podcast descriptions', () => {
      const podcastDescription = '<p>This episode covers <em>React</em> testing fundamentals and best practices for testing React components.</p>'
      
      render(<SafeHtmlRenderer htmlContent={podcastDescription} searchQuery="React" />)
  ...
  describe('CSS Classes and Styling', () => {
    ...
    it('applies default empty className when none provided', () => {
      const { container } = render(<SafeHtmlRenderer htmlContent="<p>Test content</p>" />)
